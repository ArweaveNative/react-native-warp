...
class HandlerBasedContract {
    ...
    async createBundleInteractionTx(input, options) {
        ...
        const effectiveStrict = (options === null || options === void 0 ? void 0 : options.strict) === true;
        const effectiveVrf = (options === null || options === void 0 ? void 0 : options.vrf) === true;
        if (effectiveTransfer.target != CreateContract_1.emptyTransfer.target &&
            effectiveTransfer.winstonQty != CreateContract_1.emptyTransfer.winstonQty) {
            throw new Error('Ar Transfers are not allowed for bundled interactions');
        }
        const interactionTx = await this.createInteraction(input, effectiveTags, CreateContract_1.emptyTransfer, effectiveStrict, true, effectiveVrf);
        return interactionTx;
    }
    async submitBundleInteractionTx(interactionTx) {
    ...
    async callContract(input, caller, sortKey, tags = [], transfer = CreateContract_1.emptyTransfer, strict = false, vrf = false) {
